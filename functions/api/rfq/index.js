...
const SITE_URL = 'https://hydraulicmatch.com';
...
export async function onRequest(context) {
  ...
  try {
    ...
    const partNumber = fd.get('part_number')?.toString() || '-';
    const productType = fd.get('product_type')?.toString() || '-';
    ...
    if (!resendKey) {
      ...
      return new Response(
        JSON.stringify({ ok: false, message: 'Email service not configured. Please email us at sales@hydraulicmatch.com' }),
    ...
    await sendResendEmail(resendKey, {
      ...
      subject: `New hydraulic RFQ: ${name} - ${brand} ${model} / ${quantity}`,
      text: emailBody,
    });
    ...
    if (email.includes('@') && email !== '(not provided)') {
      try {
        await sendResendEmail(resendKey, {
          ...
          replyTo: 'sales@hydraulicmatch.com',
        });
      } catch (autoReplyErr) {
