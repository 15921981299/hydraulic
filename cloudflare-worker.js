  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (!resendKey) {
        ...
        return new Response(JSON.stringify({ ok: false, message: 'Email service not configured. Please email us at sales@hydraulicmatch.com' }), {
          status: 503,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
        });
      }
      ...
      const mailResp = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Hydraulic Match <rfq@hydraulicmatch.com>',
          to: 'sales@hydraulicmatch.com',
      ...
      if (email.includes('@') && email !== '(not provided)') {
        const autoReplyBody = [
          ...
          '',
          'Helpful resources:',
          '- Hydraulic product categories: https://hydraulicmatch.com/products/',
          '- Matching process: https://hydraulicmatch.com/matching-process/',
          '',
        ...
        const autoReplyResp = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: 'Hydraulic Match <rfq@hydraulicmatch.com>',
            to: email,
            reply_to: 'sales@hydraulicmatch.com',
            subject: 'We received your hydraulic component inquiry - Hydraulic Match',
            text: autoReplyBody,
          }),
        });
      ...
      return new Response(JSON.stringify({ ok: false, message: 'Something went wrong. Please email us at sales@hydraulicmatch.com' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      });
    }
